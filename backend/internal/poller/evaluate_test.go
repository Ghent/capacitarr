...
func TestEvaluateAndCleanDisk_BelowThreshold_NoLongerClearsQueue(t *testing.T) {
	...
	result := p.evaluateAndCleanDisk(&RunAccumulator{}, group, nil, nil, 0, db.PreferenceSet{}, map[string]int{}, nil, &engine.EvaluationContext{ActiveIntegrationTypes: map[integrations.IntegrationType]bool{}})
	if result != 0 {
...
func TestEvaluateAndCleanDisk_ReconcilesDismissesStaleItems(t *testing.T) {
	...
	result := p.evaluateAndCleanDisk(&RunAccumulator{}, group, nil, nil, 0, prefs, map[string]int{}, nil, &engine.EvaluationContext{ActiveIntegrationTypes: map[integrations.IntegrationType]bool{}})
	if result != 0 {
		t.Errorf("expected 0 deletions queued, got %d", result)
	}
...
func TestEvaluateAndCleanDisk_IsSnoozed_AutoMode(t *testing.T) {
	...
	result := p.evaluateAndCleanDisk(&RunAccumulator{}, group, items, nil, 0, prefs, weights, nil, &engine.EvaluationContext{ActiveIntegrationTypes: map[integrations.IntegrationType]bool{}})
