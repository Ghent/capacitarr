...
test\:ci:
	@echo "═══ CI Test Stage ═══"
	@echo "→ [test:go] go test (Docker: golang:1.26.8-alpine)..."
	mkdir -p backend/frontend/dist && touch backend/frontend/dist/.gitkeep
	docker run --rm --pull missing -v $(CURDIR)/backend:/app $(GO_CACHE_VOLS) -w /app \
		golang:1.26.8-alpine sh -c "cd /app && go test -v ./... -count=1"
...
security\:ci:
	...
	@echo "→ [security:govulncheck] (Docker: golang:1.26.8-alpine)..."
	mkdir -p backend/frontend/dist && touch backend/frontend/dist/.gitkeep
	docker run --rm --pull missing -v $(CURDIR)/backend:/app $(GO_CACHE_VOLS) -w /app \
