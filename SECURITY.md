...
| `docs/plans/` | Inbox only (`00-active/`). Plan documents may reference example credentials in design discussions |
...
Semgrep scans all Go and Vue/TypeScript source files (every file tracked by git except the marketing site and GitHub Actions workflow YAML). Test files, utility files, and all production code are scanned.

**`.semgrepignore` exclusion — 2 directories:**
...
| `.github/workflows/` | 2 files | Semgrep auto rule `github-actions-mutable-action-tag` requires SHA-pinning every `uses:` step. That is a CI policy change and is not applied without explicit approval. Application source remains fully scanned. |

**Files skipped by Semgrep's built-in 1 MB size limit:** none. The unused root `screenshots/*.png` duplicates were removed; marketing shots live as WebP under `site/public/screenshots/` (the `site/` tree is already excluded above).
...
| `backend/internal/integrations/httpclient.go` | 81 | gosec G704 | URL is from admin-configured integration settings, not user-tainted |
| `backend/internal/integrations/httpclient.go` | 89 | gosec G706 | Sanitized URL, HTTP status code, and duration are safe to log |
| `backend/internal/integrations/httpclient.go` | 137 | gosec G704 | URL is from admin-configured integration settings, not user-tainted |
| `backend/internal/integrations/httpclient.go` | 188 | gosec G704 | URL is from admin-configured integration settings, not user-tainted |
| `backend/internal/integrations/httpclient.go` | 233 | gosec G704 | URL is from admin-configured integration settings, not user-tainted |
...
| `backend/internal/integrations/plex.go` | 243 | exhaustive | Plex API only returns `movie`, `show`, `season`, and `episode` media types |
...
| `backend/internal/services/notification_dispatch_test.go` | 327 | dupl | Test structure intentionally similar to related dispatch tests |
| `backend/internal/services/notification_dispatch_test.go` | 348 | dupl | Test structure intentionally similar to related dispatch tests |
| `backend/internal/services/poster_overlay.go` | 419 | gosec G107 | URL is from *arr metadata (PosterURL → TMDb CDN), not user input |
| `backend/internal/services/schema.go` | 218 | gosec G201 | `fmt.Sprintf("PRAGMA table_info(%s)", tableName)` — table name is a hardcoded string from within the service, never user input |
...
**Latest baseline (2026-05-29, routine hygiene scan, post v3.3.5):** 119 rules tested (53 active + 66 passive), **118 PASS, 0 FAIL, 1 WARN**
...
The full test-by-test breakdown with rule IDs is in [`docs/security/zap-baseline-20260529.md`](docs/security/zap-baseline-20260529.md). Previous baselines: [2026-04-06](docs/security/zap-baseline-20260406.md), [2026-03-24](docs/security/zap-baseline-20260324.md), [2026-03-23](docs/security/zap-baseline-20260323.md), [2026-03-16](docs/security/zap-baseline-20260316.md), [2026-03-10](docs/security/zap-baseline-20260310.md).
...
| `vite` | `8.0.16` (pinned; 8.3+ breaks `@nuxtjs/i18n` locale JSON) / `>=7.3.5` (for 7.x) | [GHSA-fx2h-pf6j-xcff](https://github.com/advisories/GHSA-fx2h-pf6j-xcff) | High | `nuxt > @nuxt/vite-builder > vite`, `vitest` |
...
**Next reassessment date:** 2026-06-28
