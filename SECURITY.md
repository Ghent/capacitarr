...
|---------------|-------|--------|
| `site/` | 38 files | Completely separate Nuxt Content marketing website with its own `package.json`, dependencies, and deployment. Has no authentication, no database, no API. The `ProseCode.vue` component uses `v-html` to render Mermaid SVG diagrams generated by the Mermaid library from trusted markdown — not user input. Would require its own security review. |
...
| `backend/internal/engine/score_test.go` | 14 | unparam | `value` is always 10 in tests but the parameter documents intent for the helper function |
...
| `backend/internal/integrations/jellystat_test.go` | 11 | gosec G101 | `testJellystatToken` is a test fixture constant, not a real credential |
